// gemm_simple4_stride_padding_fixed.cu
...
__global__ void gemm_tiled_stride(const float* A, const float* B, float* C, int M, int N, int K, int stride_A, int stride_B, int stride_C) {
...
int main() {
    ...
    std::vector<float> h_A_stride(stride_M * stride_K, 0.0f);
    std::vector<float> h_B_stride(stride_K * stride_N, 0.0f);
    ...
    gemm_tiled_stride<<<grid, block>>>(d_A, d_B, d_C, M, N, K, stride_K, stride_N, stride_N);

    cudaMemcpy(h_C_stride.data(), d_C, stride_M * stride_N * sizeof(float), cudaMemcpyDeviceToHost);

    std::cout << "Matrix C (valid ROI):" << std::endl;
    ...
    std::cout << "Matrix C (preview):" << std::endl;
    utils::print_matrix_preview_with_cut("C_ROI", h_C_stride.data(), M, N, stride_N);
